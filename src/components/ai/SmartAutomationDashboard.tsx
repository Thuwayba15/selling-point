...
export const SmartAutomationDashboard = () => {
  ...
  const loadAutomationInsights = async () => {
    ...
    try {
      ...
      const avgValue =
        opportunities.reduce((sum: number, opp: any) => sum + (opp.estimatedValue || 0), 0) /
        totalOpps;
      const highValueOpps = opportunities.filter(
        (opp: any) => (opp.estimatedValue || 0) > 50000,
      ).length;
  ...
  const handleAutoCreateOpportunity = async () => {
    try {
      ...
      const suggestion = await groqService.chat([{ role: "user", content: prompt }]);
  ...
  const handleGenerateFollowUps = async () => {
    try {
      ...
      const prioritizedOpps = followUpOpps.sort((a: any, b: any) => {
      ...
      const followUpList = prioritizedOpps
        .slice(0, 5)
        .map(
      ...
      const tasks = opportunities
        .filter((opp: any) => {
          ...
          const daysUntilClose = opp.expectedCloseDate
            ? Math.ceil(
                (new Date(opp.expectedCloseDate).getTime() - today.getTime()) /
                  (1000 * 60 * 60 * 24),
              )
            : null;
    ...
    <div className={styles.smartContainer}>
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        <Space direction="vertical" size="large">
          ...
          <Text>Intelligent CRM automation to save time and improve efficiency</Text>
          ...
          <Card title="🎯 Automation Features" className={styles.smartCard}>
            <List>
              <List.Item>
                <Text strong>Auto-Opportunity Creation:</Text> AI analyzes client history and
                suggests relevant new opportunities
              </List.Item>
              <List.Item>
                <Text strong>Intelligent Follow-up Reminders:</Text> Prioritizes opportunities by
                value and urgency
              </List.Item>
              <List.Item>
                <Text strong>Smart Task Generation:</Text> Creates stage-specific tasks with clear
                priorities
              </List.Item>
              <List.Item>
                <Text strong>Email Template Generation:</Text> AI creates personalized email
                templates
              </List.Item>
            </List>
          </Card>
